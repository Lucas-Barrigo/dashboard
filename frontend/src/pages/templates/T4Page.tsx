...
import { t4Api, projectsApi, type Project, type Template, type T4Data, type T4SecurityTest, type T4PenTest, type T4ResilienceTest, type T4SupplierFailureSim } from '../../api'
import {
...
function T4Content({ pid, sprint, template, locked, onRefresh, p4Active }: { pid: number; sprint: number; template: Template; locked: boolean; onRefresh: () => void; p4Active: boolean }) {
  ...
  const [modals, setModals] = useState({ sec: false, pen: false, res: false, sim: false })

  const load = async () => {
    const [d, st, pt, rt, sm] = await Promise.allSettled([
    ...
    else setData({ template_id: 0, tlpt_applicable: false, tlpt_completed_or_scheduled: false, tlpt_date: null, audit_trail_integrity_verified: false, incident_response_tested: false, risk_assessments_validated: false, notes: null })
  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        <CardHeader title="Testes Funcionais de Segurança" action={!locked && <Button size="sm" onClick={() => setModals(m => ({ ...m, sec: true }))}><Plus size={14} /> Adicionar</Button>} />
      ...
      <Card>
        <CardHeader title="Pen Tests" action={!locked && <Button size="sm" onClick={() => setModals(m => ({ ...m, pen: true }))}><Plus size={14} /> Adicionar</Button>} />
      ...
      <Card>
        <CardHeader title="Testes de Resiliência" action={!locked && <Button size="sm" onClick={() => setModals(m => ({ ...m, res: true }))}><Plus size={14} /> Adicionar</Button>} />
      ...
      {p4Active ? (
        <Card>
          <CardHeader title="Simulações de Falha de Fornecedor" action={!locked && <Button size="sm" onClick={() => setModals(m => ({ ...m, sim: true }))}><Plus size={14} /> Adicionar</Button>} />
    ...
    <Modal title="Novo Teste de Segurança" onClose={onClose}>
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          ...
          <FormField label="Ref Ameaça (T2)"><Input value={form.threat_ref} onChange={e => setForm(f => ({ ...f, threat_ref: e.target.value }))} placeholder="T-001" /></FormField>
        </div>
